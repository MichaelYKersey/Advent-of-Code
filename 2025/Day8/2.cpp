...
int main(int argc, char* argv[]) {
    ...
    int e1 = heap_back->first.first;
    int e2 = heap_back->first.second;
    printf("\nset_size:%d",ds.get_set_size(e1));
    printf("\n%d:%s",e1,to_string(poses[e1]).c_str());
    printf("\n%d:%s",e2,to_string(poses[e2]).c_str());
    printf("\nX product:%lld",get<0>(poses[e1])*get<0>(poses[e2]));
